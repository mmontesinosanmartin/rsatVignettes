...
In `rsat`, searching means locating the satellite images of a desired data product for a given region and time of interest (referred to as *roi* and *toi* henceforth). It requires:

1.  [**Getting access**], i.e. creating a profile in two online data portals and managing your credentials in `R`.
2.  [**Doing a search**], i.e. finding, previewing, and filtering the available information for a *roi* and *toi*.
...
The output from `sat_search()` is a `records` object. This object stores the search results metadata from the various *APIs* in standardized manner. A `records` works as a vector and every element of the vector refers to an image. Information about an image is saved in the following *slots*:
...
The first, second, and third argument in `subset()` correspond to, (1) the `records` object, (2) the value of the slot we are interested in, and (3) the name of the slot used for subsetting. The next lines of code show a more advance filtering example where a new `records` is built from images captured by both missions on the same dates:
...
The `new_rtoi()` writes automatically a new file which is a copy of the `R` object:

```{r rtoi_file}
rtoi.file <- list.files(ds.path,
                        pattern = "*.rtoi$",
                        full.name = TRUE,
                        recursive = TRUE)
rtoi.file
...
An `rtoi` is an S6 class object, so it behaves like any object in other programming languages. That is, if the `rtoi` is passed as an argument and modifies the `rtoi`, the object in the main environment is also updated. The function `sat_search()` places the resulting `records` into the `rtoi`:
...
```{r rtoi_file_update}
file.info(rtoi.file)$ctime # creation time
file.info(rtoi.file)$mtime # modification time
```
...
records(filomena) <- subset(rcd, "mod09ga", "product")
```
